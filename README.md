...
| **Inventario** | Productos por tipo y categoría (fragancia / reloj / general), lotes con CPP ponderado (FIFO), alertas de stock mínimo, fotos de producto, apertura de botella para decants, asignación masiva de categorías |
...
| **Logística** | Pipeline de pedidos locales con validación de stock, selector de frasco por pedido, señas navegables a contabilidad, comisiones, entrega y reversión |
...
| **Presupuestos** | Presupuestos PDF por cliente con análisis de rentabilidad interno y conversión a pedido; tipo Parcial con ml libre |
...
| **Catálogo** | Vista interna con filtros profesionales + exportación HTML interactiva con carrito + PDF visual con imágenes, publicación en GitHub Pages |
| **Comparador** | Solicitudes de cotización a múltiples proveedores, tabla comparativa, conversión a orden de compra |
| **Configuración** | Parámetros del negocio, tipo de cambio, logo, PIN, color de acento, CRUD de categorías dinámicas |

---

## Novedades — v2.6 (Septiembre 2026)

### Categorías dinámicas de producto

Sistema de etiquetado flexible administrado desde Configuración:

- **CRUD completo**: crear (slug inmutable), editar (nombre, tipo, emoji, color hex), eliminar con recuento de productos afectados y limpieza automática de `categoryIds`
- **Tipos**: `gender` / `olfactive` / `style` / `other`
- **Asignación individual**: selector multi-toggle en el formulario de Inventario
- **Asignación masiva**: editor tipo spreadsheet en Inventario — click para togglear por celda, save diferencial (`bulkPut` solo cambiados)
- **Visualización**: badges coloreados en tabla de Inventario y cards de Catálogo
- **Filtro**: en Inventario (select) y Catálogo (pills con color de la categoría)
- **Backup**: versión `3` del JSON incluye la tabla `categories`
- **Componente**: `src/components/ui/CategoryBadge.tsx` con `CategoryBadge` y `CategoryBadgeList`
- **DB**: schema v7 — tabla `categories` + índice `*categoryIds` MultiEntry en products

### Filtros profesionales en Catálogo

Panel estructurado con 6 secciones independientes (reemplaza las pills planas anteriores):

| Sección | Opciones |
|---------|---------|
| **Disponibilidad** | Todos / Disponibles / Sin disponibilidad |
| **Tipo de objeto** | Todos / 🌸 Fragancias / ⌚ Relojes / 📦 General |
| **Formato** | Todos / Sellados / Decants / Testers |
| **Familia olfativa** | 10 familias (se oculta al filtrar por Relojes o General) |
| **Categorías dinámicas** | Pills con colores por categoría |
| **Precio** | Rango mínimo–máximo en Guaraníes |

**"Disponibles"** = tiene stock **O** está forzado visible (`catalogVisible === true`), consistente con el toggle de visibilidad por producto.

Fila de chips activos: un chip por cada filtro activo con × individual + botón "Limpiar todo". Badge numérica en el botón Filtros con el conteo de filtros estructurados activos.

### PDF del catálogo — precios corregidos

Los precios de decants en el PDF ahora se leen de `price3/5/10/30ML` del producto (igual que la vista interna), no de `decantBatches`. Los precios son ahora consistentes entre pantalla y exportación.

### Logística — selección de frasco por pedido

Cuando hay múltiples insumos del mismo tamaño (ej: "Frasco 10ml premium" y "Frasco 10ml normal"), el sistema ya no falla silenciosamente ni bloquea el pedido por usar el frasco equivocado:

- **Selector de frasco**: aparece automáticamente en el formulario solo cuando existen 2+ opciones para el tamaño elegido. Muestra `{nombre} ({stock}u.)` o `{nombre} — sin stock`
- **Auto-selección**: al cambiar tipo a 'Decant' o al cambiar el tamaño, se pre-selecciona el frasco con más stock disponible
- **Comprometido en el pedido**: `supplyId` se guarda en `LocalOrderItem` para decants
- **Preparar**: usa el frasco comprometido; fallback al de más stock si no hay `supplyId`
- **Anular entrega**: restaura stock al frasco específico que se usó; fallback a sizeML

### Presupuestos — tipo Parcial con ml libre

El tipo "Parcial" ahora muestra un input numérico libre (step 0.5ml) en lugar del select fijo con opciones 3/5/10/30ml. El `sizeML` arranca vacío al seleccionar Parcial para que el usuario ingrese la cantidad exacta. El tipo "Decant" mantiene el select fijo.

### Ventas — corrección de edición y método de pago

**Bug IDBKeyRange al editar venta:** cuando una venta era generada desde un pedido cubierto 100% por señas (`saldo = 0`), el campo `accountId` se guardaba como `NaN` (ya que el selector de cuenta no aparecía en el modal). Al intentar editar esa venta, `db.accounts.where('id').equals(NaN)` lanzaba `DexieError: Failed to execute 'bound' on 'IDBKeyRange'`.

**Fixes aplicados:**

- **`Ventas.tsx` — `handleEditSale`**: ahora valida que `oldAccountId` y `newAccountId` sean números válidos (`!isNaN`) antes de operar sobre cuentas. Si `oldAccountId` es inválido (venta sin saldo), omite la modificación de saldo (el movimiento de las señas ya es correcto). El `paymentMethod` siempre se actualiza sin error.

- **`Logistica.tsx` — `handleDeliveryPayment`**: cuando `saldo === 0`, ahora guarda `accountId: undefined` (en lugar de `NaN`) y `paymentMethod` tomado de la seña principal (`advancePayments[0].method`). Evita que futuras ventas sin saldo queden con datos inválidos.
...
src/
├── db/
│   ├── db.ts          # Instancia Dexie + versiones del schema + seed inicial + seedCategories
...
│       ├── CategoryBadge          # Badge/pill de categoría dinámica + CategoryBadgeList
...
`pIsAvailable(p)` = `catalogVisible === true` **OR** tiene stock según su tipo. Permite que productos forzados visibles aparezcan como "Disponibles" aunque el stock sea 0.

### Frasco comprometido en logística
`LocalOrderItem.supplyId` guarda el insumo específico elegido al crear el pedido. Preparar y anular usan ese frasco, evitando discrepancias cuando hay múltiples del mismo tamaño.
...
[Opción B] Presupuesto → Pedido
   Presupuesto (borrador → enviado → aceptado)
        ↓  [botón 'Crear pedido']
   Logística — Pendiente
        ↓  [valida stock + insumos + frasco comprometido]
   Logística — En preparación  ← descuenta ML/stock, crea DecantBatch, descuenta frasco comprometido
        ↓
   Logística — Listo
